...
public class AimSwerve extends Command {
    ...
    public void execute() {
        ...
        double goalDX = targetLocation.getX() - shooterPose.getX();
        double goalDY = targetLocation.getY() - shooterPose.getY();


        double goalAngle = Math.atan2(goalDY, goalDX);
        ...
        double xVel = drivetrain.getState().Speeds.vxMetersPerSecond;
        double yVel = drivetrain.getState().Speeds.vyMetersPerSecond;


        double perpendicularVel = (perpendicularX * xVel) + (perpendicularY * yVel);
        ...
        SmartDashboard.putNumber("AS compensation heading", headingCorrection);

        double HubHeading = targetHeading.getRadians() - headingCorrection;
