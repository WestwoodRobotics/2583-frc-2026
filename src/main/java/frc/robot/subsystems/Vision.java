...
public class Vision extends SubsystemBase {
    ...
    public List<Transform3d> getObjectPos(PhotonCamera camera, Transform3d transform){
        camera.setPipelineIndex(1);

        PhotonPipelineResult result = camera.getLatestResult();
        List<PhotonTrackedTarget> objects = result.getTargets();

        List<Transform3d> positions = new ArrayList<Transform3d>();
        for(int i = 0; i < objects.size(); i++){
            Transform3d pos = transform.plus(objects.get(i).getBestCameraToTarget());
            positions.add(i, pos);
        }
